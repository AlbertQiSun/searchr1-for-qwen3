...
import logging
import warnings
# import verl  # Removed - training happens separately via train.sh
...
# Suppress Transformers library logging (silence generation flags logs)
try:
    # Prefer official Transformers logging control
    from transformers.utils import logging as hf_logging
    hf_logging.set_verbosity_error()
except Exception:
    pass

# Hard-stop any warnings emitted via Python logging from Transformers
for logger_name in [
    "transformers",
...
# Simple BM25 stub to avoid import issues
class BM25Retriever:
    ...
    def search(self, query, num=None):
        if num is None:
            num = self.top_k
        return [{"text": f"BM25 result for: {query}", "score": 0.5} for _ in range(min(num, 3))]
...
class InferenceSystem:
    ...
    def __init__(self, config: InferenceConfig):
        ...
        if self.config.use_probe:
            self._load_probe()
        self.trajectories = []  # For analysis - actual training happens via verl train.sh
    ...
    def inference(self, questions: List[Dict[str, Any]], max_turns: Optional[int] = None, probe_counters: Optional[Dict[str, int]] = None, progress_bar: Optional[tqdm] = None) -> List[Dict[str, Any]]:
        ...
        for question_data in tqdm(questions, desc="Processing questions", unit="question"):
            ...
            turn_pbar = tqdm(range(max_turns + 1), desc=f"Turns for Q{active_question['id']}", unit="turn", position=1, leave=False) if progress_bar else range(max_turns + 1)
            for turn_num in turn_pbar:
                # Update turn progress bar description
                if progress_bar and hasattr(turn_pbar, 'set_description'):
                    turn_pbar.set_description(f"Q{active_question['id']} Turn {turn_num + 1}/{max_turns + 1}")
                
                # Step 1: Generate response for this question (wrap in list for compatibility)
                active_questions_list = [active_question]
                active_questions_list, search_queries = self.inference_one_turn(active_questions_list, turn_pbar)
                active_question = active_questions_list[0]
                ...
                # Step 2: Check if completed (answered or error)
                last_turn = active_question["turns"][-1] if active_question["turns"] else {}
                ...
                if answer:
                    ...
                    active_question["final_turn"] = turn_num + 1
                    logger.info(f"Question {active_question['id']} completed in turn {turn_num + 1}")
                    break
                elif not search_query:
                    # No search query or answer found
                    active_question["error"] = "No search query or answer found"
                    active_question["final_turn"] = turn_num + 1
                    logger.info(f"Question {active_question['id']} completed with error: No search query or answer found")
                    break
                
                state = active_question["sequence"]  # 当前状态
                action = last_turn["response"]       # 动作
                # 计算奖励（示例）
                if answer:
                    metrics = calculate_metrics(answer, active_question["golden_answers"])
                    reward = metrics["f1"]
                else:
                    reward = 0.5 if search_query else -0.2  # 鼓励有效交互
                
                next_state = active_question["sequence"] + action  # 下一状态
                trajectory = {
                    "state": state,
                    "action": action,
                    "reward": reward,
                    "next_state": next_state,
                    "agent_name": "tool_agent_loop"  # 保留，支持VERL工具配置
                }
                self.trajectories.append(trajectory)

                # Note: Online training with verl happens separately via the train.sh script
                # Trajectories are collected here for analysis but training is offline
                
                # Step 3: Process retrievals if any search queries
                if search_queries:
                    active_questions_list = self.process_retrievals(active_questions_list, search_queries, probe_counters)
                    active_question = active_questions_list[0]
                
                # Step 4: Check if max turns reached
                if turn_num == max_turns - 1:
                    active_question["sequence"] += max_turn_warning
                elif turn_num == max_turns:
                    active_question["error"] = "Max turns reached"
                    active_question["final_turn"] = max_turns
                    logger.info(f"Question {active_question['id']} completed with error: Max turns reached")
                    break
            
            # Add to completed questions
            completed_questions.append(active_question)
            
            # Update main progress bar
            if progress_bar:
                progress_bar.set_description(f"Completed question {active_question['id']}" + (f" ({active_question['error']})" if active_question["error"] else ""))
